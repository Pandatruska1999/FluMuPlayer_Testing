...
class _SpatialAudioMixerState extends State<SpatialAudioMixer> {
  ...
  void _updateVolume(String trackId, double volume) {
    setState(() {
      _currentPositions[trackId] = _currentPositions[trackId]!.copyWith(
  ...
  void initState() {
    ...
    // Ensure each track in playlist has a position entry
    ...
    // Also ensure the currently playing track (from audio service) has an entry
    if (widget.audioService.currentFilePath != null) {
      _currentPositions.putIfAbsent(
        widget.audioService.currentFilePath!,
        () => AudioSourcePosition(trackId: widget.audioService.currentFilePath!),
      );
    }
  }

  // Helper method to get track info safely
  String _getTrackTitle(String trackId) {
    try {
      ...
      // If track not found in playlist, use the filename
      final parts = trackId.split('/');
      return parts.last.replaceAll('.mp3', '').replaceAll('.wav', '').replaceAll('.flac', '');
    }
  }
      ...
      child: Stack(
        children: [
          ...
          ..._currentPositions.entries.map((entry) {
            ...
            final trackTitle = _getTrackTitle(entry.key);
  ...
  Widget _buildTrackControls() {
    if (_selectedTrackId == null) {
      return const Column(
        ...
        children: [
          ...
          SizedBox(height: 8),
          Text(
            'Click on any music note in the room view',
    ...
    final trackTitle = _getTrackTitle(_selectedTrackId!);
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          runSpacing: 8,
          children: [
            _buildPositionButton('Left', Icons.arrow_left, () => _updatePosition(_selectedTrackId!, -0.1, 0, 0)),
            _buildPositionButton('Right', Icons.arrow_right, () => _updatePosition(_selectedTrackId!, 0.1, 0, 0)),
            _buildPositionButton('Front', Icons.arrow_upward, () => _updatePosition(_selectedTrackId!, 0, 0.1, 0)),
            _buildPositionButton('Back', Icons.arrow_downward, () => _updatePosition(_selectedTrackId!, 0, -0.1, 0)),
            _buildPositionButton('Up', Icons.arrow_drop_up, () => _updatePosition(_selectedTrackId!, 0, 0, 0.1)),
            _buildPositionButton('Down', Icons.arrow_drop_down, () => _updatePosition(_selectedTrackId!, 0, 0, -0.1)),
          ],
        ),
      ],
    );
  ...
  Widget _buildPositionButton(String label, IconData icon, VoidCallback onPressed) {
    return Column(
      children: [
        IconButton(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text(
              'Click on tracks to position them in 3D space around the listener',
              style: TextStyle(color: Colors.white70),
            ),
